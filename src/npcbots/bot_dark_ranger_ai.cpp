...
public:
    ...
    struct dark_ranger_botAI : public bot_ai
    {
        ...
        void CheckBlackArrow(uint32 diff)
        {
            ...
            if (Unit* target = !targets.empty() ? acore::Containers::SelectRandomContainerElement(targets) : NULL)
            {
        ...
        void SummonBotPet(Unit* from)
        {
            ...
            {
                //sLog->outError("bot_dranger_ai::SummonBotPet(): max minions");
                Unit* u = NULL;
                ...
                //if (u)
                //    sLog->outError("bot_dranger_ai::SummonBotPet(): found minion to erase(1)");
                //try 3: last resort
                if (!u)
                u = *(_minions.begin());
                //if (u)
                //    sLog->outError("bot_dranger_ai::SummonBotPet(): found minion to erase(2)");
