...
void bot_pet_ai::_calculatePos(Position& pos) const
{
    ...
    //distance
    x += (PET_FOLLOW_DIST + me->GetObjectSize()) * std::cos(o);
    y += (PET_FOLLOW_DIST + me->GetObjectSize()) * std::sin(o);
...
uint32 bot_pet_ai::GetData(uint32 data) const
{
    switch (data)
    {
        ...
        default:
            sLog->outError("bot_pet_ai::GetData(): unk data type %u!", data);
            return 0;
    }
}
...
void bot_pet_ai::SetPetStats(bool force)
{
    switch (myType)
    {
        ...
        case BOT_PET_DARK_MINION_ELITE:
            break;
        default:
            sLog->outError("bot_pet_ai::SetPetStats(): unk pet type %u, aborting", myType);
            return;
    }
    ...
    if (force)
    {
        ...
        else
        {
            ...
            sLog->outError("SetPetStats(): pInfo is NULL, setting default stats for pet %u", myType);
        }
    }
    ...
    switch (petOwner->GetBotClass())
    {
        ...
        case BOT_CLASS_WARLOCK:
            ...
            //sLog->outError("SetPetStat(): atpower += 0.57 of %i = %.2f", spdtotal, atpower);
            break;
        case BOT_CLASS_DREADLORD:
            atpower += spdtotal * 6;
            //sLog->outError("SetPetStat(): atpower += 0.57 of %i = %.2f", spdtotal, atpower);
    ...
    //sLog->outError("SetPetStat(): hp stamval %.1f, stammult %.1f, base %u, total %.2f", stamValue, stamMult, botPet->GetCreateHealth(), m_totalhp);
    ...
    if (me->getPowerType() == POWER_MANA)
    {
        ...
        float m_totalmana = intValue * intMult + me->GetCreatePowers(POWER_MANA) + (IAmFree() ? level * 25.f : 0); //+2000/+0 mana at 80
        //sLog->outError("SetPetStat(): mana intValue %.1f, intMult %.1f, base %u, total %.2f", intValue, intMult, botPet->GetCreatePowers(POWER_MANA), m_totalmana);
...
void bot_pet_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 10)
    {
        sLog->outError("bot_pet_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: %u, entry: %u)",
            int32(count), me->GetName().c_str(), uint32(petOwner->GetBotClass()), me->GetEntry());
        return;
    }
    if (!spellId)
    {
        sLog->outError("bot_pet_ai::RefreshAura(): spellId is 0 for bot %s (botclass: %u, entry: %u)",
    ...
    if (!spellInfo)
    {
        sLog->outError("bot_pet_ai::RefreshAura(): Invalid spellInfo for spell %u! Bot - %s (botclass: %u, entry: %u)",
...
void bot_pet_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        sLog->outError("bot_pet_ai::InitSpellMap(): No SpellInfo found for base spell %u", basespell);
...
void bot_pet_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        sLog->outError("bot_pet_ai::ReleaseSpellCooldown is called for wrong spell %u!", basespell);
        return;
    }
...
void bot_pet_ai::AdjustTankingPosition() const
{
    ...
    {
        if (/*!CCed(*itr) && */(*itr)->IsWithinMeleeRange(me) && !me->HasInArc(M_PI, *itr))
            ++bCount;
            //if (++bCount)
            //    break;
    }
...
void bot_pet_ai::IsSummonedBy(Unit* u)
{
    //sLog->outError("bot_pet_ai::IsSummonedBy for %s by %s", me->GetName().c_str(), u->GetName().c_str());
    //ASSERT(!petOwner);
    //ASSERT(u->GetTypeId() == TYPEID_UNIT);
    petOwner = u->ToCreature();
    myType = me->GetEntry();
...
void bot_pet_ai::OnBotPetSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotPetSpellMap::iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //sLog->outError("OnBotPetSpellInterrupted(): Adding cooldown (%u, new: %u) to spell %s (id: %u, schoolmask: %u), reqSchoolMask = %u",
...
bool bot_pet_ai::GlobalUpdate(uint32 diff)
{
    if (!petOwner)
    {
        sLog->outError("botpet:GlobalUpdate(): no owner!");
        return false;
    }
