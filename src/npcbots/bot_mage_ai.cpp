...
public:
    ...
    struct mage_botAI : public bot_ai
    {
        ...
        void EnterCombat(Unit* u) { canFrostWard = false; canFireWard = false; bot_ai::EnterCombat(u); }
 ...
 /*Special*/InitSpellMap(MANA_GEM_1);
  ...
  /*Talent*/lvl >= 50 ? InitSpellMap(SUMMON_WATER_ELEMENTAL_1) : RemoveSpell(SUMMON_WATER_ELEMENTAL_1);
        ...
        void ApplyClassPassives() const
        {
            ...
            RefreshAura(FROSTBITE3, level >= 60 ? 2 : level >= 12 ? 1 : 0);
            RefreshAura(FROSTBITE2, level >= 11 && level < 12 ? 1 : 0);
            RefreshAura(FROSTBITE1, level >= 10 && level < 11 ? 1 : 0);
            RefreshAura(ARCANE_CONCENTRATION, level >= 60 ? 2 : level >= 15 ? 1 : 0);
            ...
            RefreshAura(MOLTEN_SHIELDS, level >= 60 ? 2 : level >= 25 ? 1 : 0);
