...
void BotDataMgr::LoadNpcBots()
{
    ...
    sLog->outString("Starting NpcBot system...");
    ...
    if (result)
    {
        ...
        sLog->outString(">> Bot appearance data loaded");
    }
    else
        sLog->outString(">> Bots appearance data is not loaded. Table `creature_template_npcbot_appearance` is empty!");
    ...
    if (result)
    {
        ...
        sLog->outString(">> Bot race data loaded");
    }
    else
        sLog->outString(">> Bots race data is not loaded. Table `creature_template_npcbot_extras` is empty!");
    ...
    if (!result)
    {
        sLog->outString(">> Spawned 0 npcbots. Table `characters_npcbot` is empty!");
        allBotsLoaded = true;
        return;
    }
    ...
    sLog->outString(">> Loaded %u bot data entries",datacounter);
    ...
    for (std::list<uint32>::const_iterator itr = entryList.begin(); itr != entryList.end(); ++itr)
    {
        ...
        if (!proto)
        {
            sLog->outError("Cannot find creature_template entry for npcbot (id: %u)!", entry);
            continue;
        }
        //                                      1    2       3           4           5            6
        ...
        if (!infores)
        {
            sLog->outError("Cannot spawn npcbot %s (id: %u), not found in `creature` table!", proto->Name.c_str(), entry);
            continue;
        }
        ...
        sLog->outDebug(LOG_FILTER_NONE, ">> Spawned npcbot %s (id: %u, map: %u, grid: %u, cell: %u)", proto->Name.c_str(), entry, mapId, g.GetId(), c.GetId());
    ...
    botgrids.sort();
    botgrids.unique();
    sLog->outString(">> Spawned %u npcbot(s) within %u grid(s) in %u ms", botcounter, uint32(botgrids.size()),GetMSTimeDiffToNow(botoldMSTime));
...
void BotDataMgr::AddNpcBotData(uint32 entry, uint16 roles, uint32 faction)
{
    ...
    sLog->outError("BotMgr::AddNpcBotData(): trying to add new data but entry already exists! entry = %u", entry);
}
...
void BotDataMgr::UpdateNpcBotData(uint32 entry, NpcBotDataUpdateType updateType, void* data)
{
    ...
    switch (updateType)
    {
        ...
        default:
        {
            sLog->outError("BotDataMgr:UpdateNpcBotData: unhandled updateType %u", uint32(updateType));
            break;
        }
    }
}
