...
public:
        ...
        void DamageDealt(Unit* victim, uint32& damage, DamageEffectType damageType)
        {
            ...
            if (damage && victim != me && damageType == DIRECT_DAMAGE && victim->getPowerType() == POWER_MANA)
            {
                if (uint32 burned = std::min<uint32>(victim->GetPower(POWER_MANA), damage + me->getLevel() * 2))
                {
                    ...
                    me->CastCustomSpell(victim, FEEDBACK_EFFECT, &basepoints, NULL, NULL, true);
                }
            }
    ...
    private:
        ...
        void ProcessSpellsteal(Unit* target)
        {
            ...
            //sLog->outError("ProcessSpellsteal: on %s, fr=%u", target->GetName().c_str(), uint32(isFriend));
            ...
            for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
            {
                ...
                //sLog->outError("%s", aura->GetSpellInfo()->SpellName[0]);
            ...
            //sLog->outError("logs and auras");

            WorldPacket dataSuccess(SMSG_SPELLSTEALLOG, 8+8+4+1+4+max_dispelled*5);
            dataSuccess.append(target->GetPackGUID());      // Victim GUID
            dataSuccess.append(me->GetPackGUID());          // Caster GUID
            ...
            if (!targets.empty())
            {
                ...
                targets.remove_if(BOTAI_PRED::AuraedTargetExclude(success_list.front().first));

                randomTarget = acore::Containers::SelectRandomContainerElement(!targets.empty() ? targets : targetsCopy);
            }

            for (std::list< std::pair<uint32, uint64> >::iterator itr = success_list.begin(); itr != success_list.end(); ++itr)
            {
                ...
                if (randomTarget)
                {
                    ...
                    TransferAura(itr->first, itr->second, target, randomTarget);
                    randomTarget->CastSpell(randomTarget, SPELLSTEAL_VISUAL, true);
                }
                else
                    target->RemoveAurasDueToSpellByDispel(itr->first, SPELLSTEAL_1, itr->second, me, uint8(-1));
            }

            me->SendMessageToSet(&dataSuccess, true);
        }

        void TransferAura(uint32 spellId, uint64 casterGUID, Unit* target, Unit* newTarget)
        {
            ...
            for (Unit::AuraMap::iterator iter = range.first; iter != range.second;)
            {
                ...
                if (aura->GetCasterGUID() == casterGUID)
                {
                    ...
                    else
                    {
                        ...
                        if (Aura* newAura = Aura::TryRefreshStackOrCreate(aura->GetSpellInfo(), effMask, newTarget, NULL, &baseDamage[0], NULL, aura->GetCasterGUID()))
                        {
                            ...
                            newAura->SetLoadedState(aura->GetMaxDuration(), dur, aura->GetCharges(), aura->GetStackAmount(), recalculateMask, &damage[0]);
