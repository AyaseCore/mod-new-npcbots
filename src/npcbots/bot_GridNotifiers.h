    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
                ...
                (ai->IsInBotParty(u->GetVictim()) || me->GetVictim() == u))
            {
                if (checksteal)
                {
                    if (const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(30449)))
                        return false; //immune to steal
                }
                else if (ai->GetBotClass() == BOT_CLASS_SHAMAN)
                {
                    if (const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(370)))
...
class PolyUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_MAGE && !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(118)))//Polymorph
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_SHAMAN && !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(51514)))//Hex
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(5782)))//fear rank1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1513)))//scare beast rank1
                return true;
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN && me->GetDistance(u) < 10)//prevent break due to AOE damage
                return false;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(20066)))//repentance
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(60210)))//freezing arrow effect
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19386)))//wyvern sting rank 1
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_HUNTER &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(1991)))//scatter shot
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_ROGUE &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(2094)))//blind
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARLOCK &&
                u->GetCreatureType() == CREATURE_TYPE_HUMANOID &&
                (u->GetVictim() || u->IsControlledByPlayer()) &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6358)))//seduction
                return true;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_DREADLORD &&
                u->GetCreatureType() != CREATURE_TYPE_UNDEAD &&
                !const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(SPELL_SLEEP)))
                return true;
...
class UndeadCCUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (u->GetReactionTo(me) > REP_NEUTRAL)
                return false;
            if (const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(m_spellId)))
                return false;
...
class CastingUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            //    return false;
            if (!const_cast<Unit*>(u)->IsNonMeleeSpellCast(false,false,true))
                return false;
            ...
            if (m_spell)
            {
                ...
                if (const_cast<Unit*>(u)->IsImmunedToSpell(spellInfo))
                    return false;
...
class TranquilTargetCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            {
                if (const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(19801))) return false;//immune to tranquilizing shot
                Unit::AuraMap const &Auras = u->GetOwnedAuras();
                for (Unit::AuraMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
                {
...
class FarTauntUnitCheck
{
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            if (!free && !ai->IsInBotParty(u->GetVictim()))
                return false;
            if (me->ToCreature()->GetBotClass() == BOT_CLASS_WARRIOR && const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(355)))
                return false; //taunt
            else if (me->ToCreature()->GetBotClass() == BOT_CLASS_PALADIN && const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(62124)))
                return false; //HoR
            else if (me->ToCreature()->GetBotClass() == BOT_CLASS_DRUID && const_cast<Unit*>(u)->IsImmunedToSpell(sSpellMgr->GetSpellInfo(6795)))
                return false; //Growl
...
public:
    ...
    bool operator()(WorldObject const* ob)
    {
        ...
        {
            if (cre->IsVisible() && const_cast<Creature*>(cre)->getDeathState() == CORPSE && cre->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE) && cre->loot.isLooted() &&
...
namespace BOTAI_PRED
{
    ...
    {
        public:
            AuraedTargetExcludeByCaster(uint32 spellId, uint64 caster) : _spellId(spellId), _caster(caster) {}
            bool operator()(Unit const* target)
            {
