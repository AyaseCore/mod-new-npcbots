...
public:
    ...
    struct water_elemental_botpetAI : public bot_pet_ai
    {
        ...
        void JustDied(Unit* u) { me->IsAIEnabled = false; me->ToTempSummon()->UnSummon(3000); bot_pet_ai::JustDied(u); }
        ...
        void DoPetActions(uint32 diff)
        {
            ...
            if (!target || target->isFrozen() || target->GetDistance(me) - target->GetObjectSize() > 25.f ||
