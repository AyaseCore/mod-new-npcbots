...
public:
    ...
    struct priest_botAI : public bot_ai
    {
        ...
        bool HealTarget(Unit* target, uint32 diff)
        {
            ...
            //sLog->outError("priest_bot:HealTarget(): %s's pct %u, hppctps %i, epct %i",
        ...
        void CheckMending(uint32 diff)
        {
            ...
            for (GroupReference const* itr = gr->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                if (player->HaveBot())
                {
                    ...
                    for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
                    {
                        if (gr->IsMember(itr->first) && itr->second->IsAlive() && !itr->second->getAttackers().empty() &&
  ...
  /*Talent*/lvl >= 40 ? InitSpellMap(POWER_INFUSION_1) : RemoveSpell(POWER_INFUSION_1);
        ...
        void InitHeals()
        {
            ...
            if (InitSpell(me, HEAL))
            {
                ...
                _heals[HEAL] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3));
            }
            else
                _heals[HEAL] = 0;
            ...
            if (InitSpell(me, FLASH_HEAL_1))
            {
                ...
                _heals[FLASH_HEAL_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3));
            }
            else
            ...
            if (InitSpell(me, PENANCE_HEAL_1))
            {
                ...
                _heals[PENANCE_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3));
            }
            else
                _heals[PENANCE_1] = 0;
            ...
            if (InitSpell(me, DESPERATE_PRAYER_1))
            {
                ...
                _heals[DESPERATE_PRAYER_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3));
            }
            else
