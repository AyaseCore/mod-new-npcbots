...
public:
    ...
    struct sphynx_botAI : public bot_ai
    {
        ...
        void EnterCombat(Unit* u) { dmgReceived = 0; DraincheckTimer = 2000; bot_ai::EnterCombat(u); }
        ...
        void JustDied(Unit* u) { dmgReceived = 0; bot_ai::JustDied(u); }
        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& damageinfo, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool crit) const
        {
            ...
            if (baseId == SPLASH_ATTACK_1)
                fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * (spellInfo->Effects[1].BonusMultiplier - 1.f) * me->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->CalculateLevelPenalty(spellInfo);
        ...
        void OnClassSpellGo(SpellInfo const* spellInfo)
        {
            ...
            if (baseId == DEVOUR_MAGIC_1)
            {
                ...
                if (dispelsDealt > 0)
                {
                    ...
                    me->HealBySpell(me, spellInfo, healthGain);
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
                ...
                !target->IsTotem() && me->GetReactionTo(target) <= REP_NEUTRAL)
            {
                ...
                me->DealDamage(me, target, damage, NULL, SPELL_DIRECT_DAMAGE, devInfo->GetSchoolMask(), devInfo);
        ...
        void OnBotDispelDealt(Unit* dispelled, uint8 num)
        {
