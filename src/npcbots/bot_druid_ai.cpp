...
public:
    ...
    struct bot_druid_ai : public bot_ai
    {
        ...
        void JustDied(Unit* u) { removeShapeshiftForm(); UnsummonAll(); bot_ai::JustDied(u); }
        ...
        bool removeShapeshiftForm()
        {
            ...
            //if (form != FORM_NONE)
            {
                ...
                {
                    //sLog->outError("druid_bot::removeShapeshiftForm(): still has poweType %u!", uint32(me->getPowerType()));
                    me->setPowerType(POWER_MANA);
                }
                if (me->GetShapeshiftForm() != FORM_NONE)
                {
                    //sLog->outError("druid_bot::removeShapeshiftForm(): still speshifted into %u!", uint32(me->GetShapeshiftForm()));
        ...
        void CheckBattleRez(uint32 diff)
        {
            ...
            if (Unit* targetOrCorpse = !targets.empty() ? acore::Containers::SelectRandomContainerElement(targets) : NULL)
            {
        ...
        void setStats(BotStances form)
        {
            ...
            switch (form)
            {
                case DRUID_BEAR_FORM:
                    ...
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_RAGE");
                ...
                case DRUID_CAT_FORM:
                    ...
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_ENERGY");
                ...
                case DRUID_MOONKIN_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (moonkin)");
                        me->setPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TREE_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (tree)");
                        me->setPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TRAVEL_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (travel)");
                        me->setPowerType(POWER_MANA);
                    }
                    break;
                case DRUID_AQUATIC_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (aquatic)");
                        me->setPowerType(POWER_MANA);
                    }
                    break;
                ...
                //    if (me->getPowerType() != POWER_MANA)
                //    {
                //        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (flight)");
                //        me->setPowerType(POWER_MANA);
                //    }
                //    break;
                case BOT_STANCE_NONE:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //sLog->outError("druid_bot::setStats(): has to set powerType to POWER_MANA (deshape)");
                        me->setPowerType(POWER_MANA);
                    }
                    break;
                default:
                    sLog->outError("druid_bot::setStats(): NYI form %u", uint32(form));
                    setStats(BOT_STANCE_NONE);
                    return;
            }
        ...
        void ApplyClassDamageMultiplierMelee(uint32& /*damage*/, CalcDamageInfo& damageinfo) const
        {
            ...
            damageinfo.damage = damageinfo.damage * (1.0f + pctbonus);
        }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (_form == DRUID_CAT_FORM)
            {
                    ...
                    baseId == RAKE_1 || baseId == RAVAGE_1 || baseId == SHRED_1)
                {
                    comboPoints++;
                    //debug
                    //sLog->outError("druid_bot CP GEN: %s adds 1, now %u", spell->SpellName[0], uint32(comboPoints));
                    if (primalFuryProc)
                    {
                        comboPoints++;
                        //debug
                        //sLog->outError("druid_bot CP EX: now %u", uint32(comboPoints));
                    }
                    if (comboPoints > 5)
                    {
                        comboPoints = 5;
                        //debug
                        //sLog->outError("druid_bot CP NOR: now %u", uint32(comboPoints));
                    }
                }
                ...
                {
                    //debug
                    //sLog->outError("druid_bot CP SPEND1: %u to 0", uint32(comboPoints));
                    if (lvl >= 25 && comboPoints > 0)
                    {
                        if (urand(1,100) <= uint32(comboPoints * 20))
                        {
                            ...
                            //debug
                            //sLog->outError("druid_bot CP SPEND1: PS proc!");
                        }
                    }
                    comboPoints = 0;
                }
        ...
        void JustSummoned(Creature* summon)
        {
            if (summon->GetEntry() == BOT_PET_FORCE_OF_NATURE)
            {
                ...
                if (!found)
                {
                    sLog->outError("Druid_bot:JustSummoned() treants array is full");
                    ASSERT(false);
                }
            }
        }
        ...
        void SummonedCreatureDespawn(Creature* summon)
        {
            ...
            if (summon->GetEntry() == BOT_PET_FORCE_OF_NATURE)
            {
                ...
                if (!found)
                {
                    sLog->outError("Druid_bot:SummonedCreatureDespawn() treant is not found in array");
                    ASSERT(false);
                }
            }
        }
        ...
        void UnsummonAll()
        {
            ...
            {
                if (_treants[i])
                    if (Unit* tr = sObjectAccessor->FindUnit(_treants[i]))
