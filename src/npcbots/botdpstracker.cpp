...
void DPSTracker::_Release()
{
    for (DamageTakenMap::const_iterator itr = _damages.begin(); itr != _damages.end(); ++itr)
    {
        ...
        //sLog->outError("DPSTracker::Release(): guidlow = %u, time = %u, tick damage %u, total %u, dps = %u",
...
{
    //sLog->outError("DPSTracker::OnDamage(): on %s, damage %u", victim->GetName().c_str(), damage);
...
uint32 DPSTracker::GetDPSTaken(uint64 guid) const
{
    ...
    //sLog->outError("DPSTracker::GetDPSTaken(): from %u, damage %u", guid, itr != _DPSes.end() ? itr->second : 0);
