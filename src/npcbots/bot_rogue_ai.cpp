...
public:
    ...
    struct rogue_botAI : public bot_ai
    {
        ...
        void JustDied(Unit* u) { comboPoints = 0; bot_ai::JustDied(u); }
        ...
        void Attack(uint32 diff)
        {
            ...
            {
                //sLog->outError("rogue_bot: cannot attack target...");
                return;
            }
            ...
            if (dist > 5)
            {
                ...
                //    sLog->outError("rogue_bot: cannot attack target (dist)...");
                return;
            }
            ...
            {
                //sLog->outError("bot_rogue: delaying attacks on gouged or blinded target...");
                return;
            }
            ...
            if (GetSpell(FAN_OF_KNIVES_1) && energy >= ecost(FAN_OF_KNIVES_1) &&
                ...
                + 50*(me->GetAuraEffect(SPELL_AURA_MOD_MELEE_HASTE, SPELLFAMILY_ROGUE, 0x40000000, 0x0, 0x0) != NULL)
            ...
            if (spellInfo->NeedsComboPoints() && comboPoints)
            {
                if (lvl >= 10)
                {
                    if (irand(1,100) <= 20 * comboPoints)
                    {
                        ...
                        //sLog->outError("rogue_bot CP SPEND1: RS proc!");
                    }
                }
            }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            {
                ++comboPoints;
                //sLog->outError("rogue_bot CP GEN2: %s adds 1, now %u", spell->SpellName[0], uint32(comboPoints));
                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //sLog->outError("rogue_bot CP NOR2: now %u", uint32(comboPoints));
                }
            }
                ...
                baseId == AMBUSH_1 || baseId == GARROTE_1 || baseId == CHEAP_SHOT_1/* || baseId == SHIV_1*/)
            {
                ...
                //sLog->outError("rogue_bot CP GEN1: %s adds %u, now %u",
                ...
                {
                    ++comboPoints;
                    //sLog->outError("rogue_bot CP GEN1: glyphSS proc, now %u", uint32(comboPoints));
                }

                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //sLog->outError("rogue_bot CP NOR1: now %u", uint32(comboPoints));
                }
            }
            ...
            else if (spell->NeedsComboPoints() && comboPoints)
            {
                ...
                //sLog->outError("rogue_bot CP SPEND1: %u to 0", tempCP);
                ...
                if (lvl >= 15)
                {
                    if (urand(1,100) <= 60)
                    {
                        ...
                        //sLog->outError("rogue_bot CP SPEND1: RU proc!");
                    }
                }
            }
            ...
            {
                //sLog->outError("rogue_bot Preparation hit!");
