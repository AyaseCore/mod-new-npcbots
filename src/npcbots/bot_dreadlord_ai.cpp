    ...
    public:
        ...
        void UpdateAI(uint32 diff)
        {
                ...
                me->GetPower(POWER_MANA) >= INFERNAL_COST && Rand() < 60)
            {
                ...
                if (target)
                    target->GetPosition(&_infernoPos);
                else
                    me->GetNearPoint(NULL, _infernoPos.m_positionX, _infernoPos.m_positionY, _infernoPos.m_positionZ, 0.f, 5.f, 0.f);

                me->CastSpell(_infernoPos.GetPositionX(), _infernoPos.GetPositionY(), _infernoPos.GetPositionZ(), GetSpell(INFERNO_1), false);
                return;
            }
        ...
        void OnClassSpellGo(SpellInfo const* spellInfo)
        {
            ...
            if (baseId == INFERNO_1)
            {
                me->CastSpell(_infernoPos.GetPositionX(), _infernoPos.GetPositionY(), _infernoPos.GetPositionZ(), SPELL_INFERNO_METEOR_VISUAL, true);
        ...
        void OnBotDamageDealt(Unit* victim, uint32 damage, CleanDamage const* /*cleanDamage*/, DamageEffectType /*damagetype*/, SpellInfo const* spellInfo)
        {
            ...
            if (damage && victim != me && spellInfo && spellInfo->GetFirstRankSpell()->Id == CARRION_SWARM_1)
            {
                ...
                //sLog->outError("OnBotDamageDealt(drl): %s on %s base val %i (%s),",
                //    me->GetName().c_str(), victim->GetName().c_str(), int32(damage), spellInfo->SpellName[0]);
                me->CastCustomSpell(me, SPELL_TRIGGERED_HEAL, &basepoints0, NULL, NULL, true);
            }
        }
