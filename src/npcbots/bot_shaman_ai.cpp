...
public:
    ...
    struct shaman_botAI : public bot_ai
    {
        ...
        void EnterCombat(Unit* u) { TotemsCheckTimer = 0; bot_ai::EnterCombat(u); }
        ...
        void JustDied(Unit* u) { UnsummonAll(); removeShapeshiftForm(); bot_ai::JustDied(u); }
        ...
        void CheckTotems(uint32 diff)
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                    ...
                    me->GetDistance(_totems[i].second._pos) > _totems[i].second._effradius)
                {
                    ...
                    if (!to)
                    {
                        sLog->outError("%s has unexpectingly lost totem in slot %u!", me->GetName().c_str(), i);
                        _totems[i].first = 0;
                        continue;
                    }
                    to->ToTotem()->UnSummon();
        ...
        void CheckFireNova(uint32 diff)
        {
            ...
            std::list<Unit*> targets;
            GetNearbyTargetsList(targets, 9.f, 0, ObjectAccessor::GetObjectInWorld(_totems[T_FIRE].first, (Creature*)NULL));
        ...
        void ApplyClassDamageMultiplierHeal(Unit const* /*victim*/, float& heal, SpellInfo const* spellInfo, DamageEffectType damagetype, uint32 stack) const
        {
            ...
            if (lvl >= 45)
                flat_mod += GetTotalBotStat(STAT_INTELLECT) * 0.15f * me->CalculateDefaultCoefficient(spellInfo, damagetype) * stack * 1.88f * me->CalculateLevelPenalty(spellInfo) * stack;
        ...
        void JustSummoned(Creature* summon)
        {
            if (summon->GetEntry() == BOT_PET_SPIRIT_WOLF)
            {
                ...
                if (!found)
                {
                    sLog->outError("Shaman_bot:JustSummoned() wolves array is full");
                    ASSERT(false);
                }
            }
        }
        ...
        void SummonedCreatureDespawn(Creature* summon)
        {
            ...
            if (summon->GetEntry() == BOT_PET_SPIRIT_WOLF)
            {
                ...
                if (!found)
                {
                    sLog->outError("Shaman_bot:SummonedCreatureDespawn() wolf is not found in array");
                    ASSERT(false);
                }
            }
        }
        ...
        void UnsummonAll()
        {
            ...
            {
                if (_wolves[i])
                    if (Unit* wo = sObjectAccessor->FindUnit(_wolves[i]))
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != 0)
                {
                    ...
                    if (!to)
                    {
                        //sLog->outError("%s has no totem in slot %u during remove!", me->GetName().c_str(), i);
                        continue;
                    }
                    to->ToTotem()->UnSummon();
                }
        ...
        void OnBotDespawn(Creature* summon)
        {
            if (!summon)
            {
                sLog->outError("OnBotDespawn(): Shaman bot %s received NULL", me->GetName().c_str());
                ASSERT(false);
                //UnsummonAll();
                return;
            }
            ...
            {
                //sLog->outError("OnBotDespawn(): Shaman bot %s has despawned summon %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Id)
            {
                ...
                default:
                    sLog->outError("OnBotDespawn(): Shaman bot %s has despawned totem %s in unknown slot %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }

            if (_totems[slot].first == 0)
                sLog->outError("OnBotDespawn(): Shaman bot %s has despawned totem %s while not having it registered!", me->GetName().c_str(), summon->GetName().c_str());
            else if (_totems[slot].second._type == BOT_TOTEM_NONE || _totems[slot].second._type >= BOT_TOTEM_END)
                sLog->outError("OnBotDespawn(): Shaman bot %s has despawned totem %s with no type assigned!", me->GetName().c_str(), summon->GetName().c_str());

            //here we reset totem category cd (not totem spell cd) if totem is destroyed
            //sLog->outError("OnBotDespawn(): %s despawned (%s : %u)", summon->GetName().c_str(), summon->IsAlive() ? "alive" : summon->isDying() ? "justdied" : "unk", (uint32)summon->getDeathState());
        ...
        void OnBotSummon(Creature* summon)
        {
            ...
            if (!totem || !totem->IsTotem())
            {
                //sLog->outError("OnBotSummon(): Shaman bot %s has summoned creature %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Id)
            {
                ...
                case SUMMON_TYPE_TOTEM_AIR:     slot = T_AIR;   break;
                default:
                    sLog->outError("OnBotSummon(): Shaman bot %s has summoned totem %s with unknown type %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }
            ...
            //sLog->outError("shaman bot: summoned %s (type %u) at x=%.2f, y=%.2f, z=%.2f",
  ...
  /*Talent*/lvl >= 60 ? InitSpellMap(FERAL_SPIRIT_1) : RemoveSpell(FERAL_SPIRIT_1); //not casted
        ...
        void ApplyClassPassives() const
        {
            ...
            RefreshAura(MAELSTROM_WEAPON5, level >= 70 ? 2 : level >= 59 ? 1 : 0);
